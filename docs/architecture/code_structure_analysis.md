...
- **In-Flight Request Deduplication**: When multiple clients connect or deep-link simultaneously, the `SocketService` tracks active database fetch promises in an in-memory Map (`partnerRequests`). Subsequent duplicate requests subscribe to the _same_ pending Promise rather than triggering duplicate MongoDB queries, saving database connections.
