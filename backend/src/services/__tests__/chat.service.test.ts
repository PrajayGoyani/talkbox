...
describe("ChatService", () => {
  ...
  describe("getChatMessages (Scrubbing)", () => {
    it("should scrub old messages for Free users", async () => {
      ...
      vi.mocked(Chat.findById).mockResolvedValue({ _id: chatId, status: "accepted", userA: userId, userB: "other" } as any);
      vi.mocked(User.findById).mockReturnValue({
        select: vi.fn().mockResolvedValue({ plan: "free" } as any),
      } as any);
      ...
      const result = await chatService.getChatMessages(chatId, userId, 50, null);
    ...
    it("should NOT scrub old messages for Pro users", async () => {
      ...
      vi.mocked(Chat.findById).mockResolvedValue({ _id: chatId, status: "accepted", userA: userId, userB: "other" } as any);
      vi.mocked(User.findById).mockReturnValue({
        select: vi.fn().mockResolvedValue({ plan: "pro" } as any),
      } as any);
      vi.mocked(Message.find).mockReturnValue({
        sort: vi.fn().mockReturnThis(),
        limit: vi.fn().mockResolvedValue(mockMessages as any),
