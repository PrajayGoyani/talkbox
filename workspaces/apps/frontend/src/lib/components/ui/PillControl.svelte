...
<div class={cn("flex items-center gap-2 overflow-x-auto no-scrollbar py-1 select-none", className)}>
      ...
      {#if option.badge !== undefined && option.badge !== null && option.badge !== 0}
        <span
          class={cn(
            ...
            active ? "bg-white text-indigo-600" : "bg-slate-200 dark:bg-white/10 text-slate-600 dark:text-slate-400",
          )}
        >
          {option.badge}
        </span>
