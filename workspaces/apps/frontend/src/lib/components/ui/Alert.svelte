<script lang="ts">
  ...
  const config = $derived(
    variants[alert.type as keyof typeof variants] || variants.success,
  );
</script>
...
>
  <div
    class="flex items-center justify-center w-10 h-10 rounded-xl bg-white/20 shrink-0 text-white shadow-inner"
  >
  ...
  <div class="flex flex-col flex-1 min-w-0">
    <span
      class="text-xs uppercase tracking-widest font-black text-white/60 mb-0.5"
    >
      {config.label}
    </span>
